...
run("Stream.broadcast({ memory: true })", [1, 2, 2, 3, 3, 'done-1', 4, 'done-2', 'done-3', 'done-4'], function(test, done) {
    var stream    = Stream.of(0);
    stream.push(1);
...
run("Stream.broadcast({ hot: true })", [2, 3, 'done-1'], function(test, done) {
    var stream    = Stream.of(0);
    stream.push(1);

    var broadcast = stream
    // This is never called, as hot stream remains live
    .done(() => test('never called'))
    .broadcast({ hot: true });
